...
export default function ModelPricingEditor({
  ...
  const rowSelection = {
    ...
    onChange: (selectedRowKeys) => {
      setSelectedModelNames(selectedRowKeys);
      if (selectedRowKeys.length > selectedModelNames.length) {
        const newlyAdded = selectedRowKeys.find((key) => !selectedModelNames.includes(key));
        if (newlyAdded) {
          setSelectedModelName(newlyAdded);
        }
      }
    },
  };
          ...
          >
            <div style={{ overflowX: 'auto' }}>
              <Table
                ...
                onRow={(record) => ({
                  style: {
                    background: record.name === selectedModelName
                      ? 'var(--semi-color-primary-light-default)'
                      : selectedModelNames.includes(record.name)
                        ? 'var(--semi-color-success-light-default)'
                        : undefined,
                    boxShadow: record.name === selectedModelName
