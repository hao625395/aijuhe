...
const TopUp = () => {
  ...
  const preTopUp = async (payment, amountOverride) => {
    const requestedTopUpCount =
      amountOverride === undefined ? topUpCount : Number(amountOverride);
    if (amountOverride !== undefined) {
      setTopUpCount(requestedTopUpCount);
      setSelectedPreset(requestedTopUpCount);
    }
    ...
    try {
      ...
      await requestAmountByPayment(payment, requestedTopUpCount);

      if (requestedTopUpCount < selectedMinTopUp) {
